...
void imu_handler(const sensor_msgs::msg::Imu::ConstSharedPtr msg_in)    // IMU数据处理回调函数
{
    ...
    msg_store.angular_velocity.x = x2;               // 存储转换后的角速度X分量
    msg_store.angular_velocity.y = y2;               // 存储转换后的角速度Y分量
    msg_store.angular_velocity.z = z2;               // 存储转换后的角速度Z分量
    msg_store.linear_acceleration.x = acc_x2;        // 存储转换后的加速度X分量
    msg_store.linear_acceleration.y = acc_y2;        // 存储转换后的加速度Y分量
    msg_store.linear_acceleration.z = acc_z2;        // 存储转换后的加速度Z分量
...
void serialize_to_file(){                           // 将标定结果序列化到文件的函数
    ...
    std::string file_path = std::string(homeDir) + "/桌面/imu_calib_data.yaml";  // 构建文件路径
...
int main(int argc, char** argv)                   // 主函数
{
    ...
    auto nh = rclcpp::Node::make_shared("calibrate_imu");    // 创建节点
    ...
    auto subImu = nh->create_subscription<sensor_msgs::msg::Imu>("/utlidar/imu", 300,  imu_handler);  // 创建IMU数据订阅者
