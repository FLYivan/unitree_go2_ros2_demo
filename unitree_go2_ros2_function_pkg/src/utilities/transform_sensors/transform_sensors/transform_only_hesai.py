...
class Repuber(Node):  # 定义传感器转换节点类
    ...
    def voxel_grid_downsample(self, points):
        """使用PCL的VoxelGrid进行点云降采样"""
        # 将点云数据转换为PCL点云格式
        cloud = pcl.PointCloud()
        # 只取XYZ坐标用于降采样
        xyz_points = np.array([[p[0], p[1], p[2]] for p in points], dtype=np.float32)
        cloud.from_array(xyz_points)
        ...
        # 将降采样后的点云转回原来的格式
        filtered_xyz = cloud_filtered.to_array()
        downsampled_points = []
        
        # 为降采样后的每个点找到最近的原始点，保留其非空间属性
        for xyz in filtered_xyz:
            # 找到最近的原始点
            distances = np.linalg.norm(np.array([[p[0], p[1], p[2]] for p in points]) - xyz, axis=1)
            nearest_idx = np.argmin(distances)
            # 创建新点，包含降采样后的空间坐标和原始点的其他属性
            new_point = points[nearest_idx].copy()
            new_point[0:3] = xyz
            downsampled_points.append(new_point)
    ...
    def cloud_callback(self, data):  # 点云回调函数
        ...
        # 使用PCL进行体素网格降采样
        downsampled_points = self.voxel_grid_downsample(transformed_points)
        
        # 应用时间偏移到转换后的点云消息
        # 新时间戳 = 原始时间戳 + 时间偏移量
        elevated_cloud = pc2.create_cloud(data.header, data.fields, downsampled_points)  # 创建新的点云消息
