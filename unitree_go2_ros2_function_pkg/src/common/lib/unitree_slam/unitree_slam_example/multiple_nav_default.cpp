...
int main(int argc, const char **argv)
{
    ...
    send_msg.seq_().data() = "index:123;"; // 设置消息的序列号为123，用户可以设置为1到10000之间的值
    send_msg.command_() = 10;              // 设置命令为10，表示多节点循环导航的默认命令

    publisher->Write(send_msg); // 发布消息
    std::cout << "send multiple nav default command" << std::endl; // 打印发送多节点默认导航命令的信息
    sleep(2); // 休眠2秒
