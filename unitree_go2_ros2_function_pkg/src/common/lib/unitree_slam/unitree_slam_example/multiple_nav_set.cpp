...
int main(int argc, const char **argv)
{
    ...
    send_msg.command_() = 11;              // 11多节点循环导航（设置）命令

    send_msg.node_edge_name_().push_back(0); // 目标点名称
    send_msg.node_edge_name_().push_back(3); // 目标点名称

    publisher->Write(send_msg); // 发布消息
    std::cout << "send multiple nav set command" << std::endl; // 打印发送多节点导航设置命令的信息
    sleep(2); // 休眠2秒
