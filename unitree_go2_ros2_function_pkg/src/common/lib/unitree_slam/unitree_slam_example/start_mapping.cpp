...
int main(int argc, const char **argv)
{
    ...
    send_msg.command_() = 3;               // 3是开始映射的命令
    send_msg.attribute_() = 0; //GO2: 当这个值为1时，XT16激光雷达节点被启用，2时，MID360激光雷达节点被启用。请根据实际激光雷达情况确认值。  B2:请提供一个固定的0值。
    send_msg.seq_().data() = "index:123;"; // 用户设置的值123（1-10000）。

    publisher->Write(send_msg); // 发布消息
    std::cout << "send start mapping command" << std::endl; // 打印发送开始映射命令的信息
    sleep(2); // 休眠2秒
