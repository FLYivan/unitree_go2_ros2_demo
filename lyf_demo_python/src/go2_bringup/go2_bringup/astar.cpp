  ...
  const float & tolerance)
{
  ...
  if (!areInputsValid()) {
    RCLCPP_ERROR(_logger, "Failed to create path: inputs are invalid");
    return false;
  }
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    if (iterations % _timing_interval == 0) {
      ...
      if (static_cast<double>(planning_duration.count()) >= _max_planning_time) {
        RCLCPP_ERROR(_logger, "Failed to create path: planning timeout reached");
        return false;
      }
    }
  ...
  RCLCPP_ERROR(_logger, "Failed to create path: no valid path found and exceeded tolerance threshold");
  return false;
}
