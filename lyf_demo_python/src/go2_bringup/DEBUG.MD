...
3、测试定时器重启最小值（1、同步将return改break，2、超时加入黑名单的，也添加打印信息，以便区别）
...
为什么自动切换路径就卡死，定时器启动就有了？——aborted的时候，定时器启动才会再次探索
...
5、超时加入黑名单的，也添加打印信息，以便区别

3、explore超时了，会触发什么？——加入黑名单，并生成新计划
